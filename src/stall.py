...
class STALL:
    ...
    def __init__(
        ...
        self.frequency_wt_mean = None
        self.frequency_wt_matrix = None
        self.frequency_calib_sorted = None

        # Load Frequency Branch calibration
        frequency_params_path = os.path.join(
            _REPO_ROOT,
            "precomputed",
            "frequency_params.npz"
        )

        if os.path.exists(frequency_params_path):

            frequency_data = np.load(
                frequency_params_path
            )

            self.frequency_wt_mean = frequency_data[
                "frequency_mean"
            ]

            self.frequency_wt_matrix = frequency_data[
                "frequency_W"
            ]

            self.frequency_calib_sorted = frequency_data[
                "frequency_calib_sorted"
            ]

            print("Frequency calibration loaded successfully!")

        else:
            print("Frequency calibration file not found.")
    ...
    def extract_frequency_features(self, frames):
        """
        Extract normalized low- and high-frequency energy
        from each video frame using 2D FFT.
        ...
        for frame in frames:
            ...
            # Frequency energy
            low_energy = np.mean(
                magnitude[low_mask] ** 2
            )

            high_energy = np.mean(
                magnitude[high_mask] ** 2
            )

            # Normalize the two energies
            total_energy = (
                low_energy +
                high_energy +
                1e-8
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # Frequency branch scoring
    # ─────────────────────────────────────────────────────────────────────────

    def frequency_score(self, frames):
        """
        Calculate the Frequency Branch score for one video.
        """

        # Extract FFT frequency features
        features = self.extract_frequency_features(frames)

        # Apply the saved whitening transformation
        whitened = whitening_transform(
            features,
            self.frequency_wt_mean,
            self.frequency_wt_matrix
        )

        # Gaussian log-likelihood
        ll = log_likelihood(whitened)

        # One score for the complete video
        video_score = np.mean(ll)

        # Convert to percentile using real-video calibration
        percentile = get_percentile_score(
            np.array([video_score]),
            self.frequency_calib_sorted
        )

        return video_score, percentile

    def _scores_from_embs(self, embs, frames=None):
        """Compute Spatial + Temporal + Frequency scores."""
        ...
        spat_agg = AGG_STR2FN[self.spat_agg](
            spat_ll,
            axis=1
        )

        spat_pct = get_percentile_score(
            spat_agg,
            self.calib_spat_sorted
        )
        ...
        temp_agg = AGG_STR2FN[self.temp_agg](
            temp_ll,
            axis=1
        )

        temp_pct = get_percentile_score(
            temp_agg,
            self.calib_temp_sorted
        )
        ...
        frequency_score_value = None
        frequency_pct = None

        if (
            frames is not None
            and self.frequency_wt_mean is not None
            and self.frequency_wt_matrix is not None
            and self.frequency_calib_sorted is not None
        ):

            frequency_score_value, frequency_pct = (
                self.frequency_score(frames)
            )
        ...
        if frequency_pct is not None:
            ...
            # Original STALL score if Frequency
        ...
        return {
            ...
            "frequency_score": frequency_score_value,
